...
SELECT sex.sex_name AS sex, ROUND (SUM(mortality.deaths_estimate)) AS deaths_estimate
FROM sex JOIN mortality ON (sex.sex_id = mortality.sex)
WHERE mortality.locality IN ( SELECT fips_county
							  FROM locality
							  WHERE locality_name LIKE 'Alabama%' )
	 AND mortality.tumor IN ( SELECT tumor_id
							  FROM tumor 
							  WHERE tumor_name = 'Pancreatic cancer')
...
FROM sex JOIN mortality ON (sex.sex_id = mortality.sex)
	 JOIN tumor ON (tumor.tumor_id = mortality.tumor)
...
FROM mortality
WHERE mortality.tumor IN ( SELECT tumor_id
						   FROM tumor 
						   WHERE tumor_name = 'Ovarian cancer' OR tumor_name = 'Cervical cancer' ) 
...
WHERE mortality.tumor IN ( SELECT tumor_id
						   FROM tumor 
						   WHERE tumor_name = 'Testicular cancer' OR tumor_name = 'Prostate cancer' ) 
GROUP BY mortality.years;	
...
WHERE mortality.locality IN ( SELECT fips_county
							  FROM locality
							  WHERE locality_name LIKE 'California%' )
